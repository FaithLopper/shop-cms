...
import RanksUpdateForm from "../../compoments/ranks/RanksUpdateForm";

class RanksUpdate extends SaveBasePage {
  constructor(props) {
    super(props);
    const { t } = this.props;
    this.objectName = t("objectName");
    this.getListUrl = sitePathConfig.ranks.path;
    this.actionFooter = false;
    this.breadcrumbs = [
      {
        name: t("breadcrumbs.parentPage"),
        path: sitePathConfig.ranks.path,
      },
      {
        name: this.isEditing
  ...
  onInsertCompleted = (res) => {
    ...
    if (res?.result) {
      this.showSuccessConfirmModal({
        onContinueEdit: () => {
          history.push(
            sitePathConfig.ranks.childrenKeys[1].replace(":id", res.id)
          );
        },
      });
...
export default connect(
  mapStateToProps,
  mapDispatchToProps
)(withTranslation(["ranksListPage", "listBasePage"])(RanksUpdate));
