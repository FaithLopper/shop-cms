...
class CategoryListPage extends ListBasePage {
    ...
    constructor(props) {
        ...
        this.objectName = t("objectNam");

        this.breadcrumbs = [
            {
                name: `${t("breadcrumbs.currentPage")} (${parentName})`,
                path: `${sitePathConfig.category.path}${this.handleRoutingParent()}`
            },
        ];
        this.columns = [
        {
    ...
    componentWillMount() {
        ...
        this.userData = actions.getUserData();
...
export default connect(mapStateToProps, mapDispatchToProps)(withTranslation(['categoryListPage','listBasePage','constants', 'basicModal'])(CategoryListPage));
