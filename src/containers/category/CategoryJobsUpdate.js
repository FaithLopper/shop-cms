...
class CategoryJobsUpdate extends SaveBasePage {
  constructor(props) {
    super(props);
    const { t } = this.props;
    this.objectName = `${t("objectName")} ${t("kind.jobs")}`;
    this.getListUrl = sitePathConfig.categoryJobs.path;
    this.actionFooter = false;
    this.breadcrumbs = [
      {
        name: `${t("breadcrumbs.parentPage")} ${t("kind.jobs")}`,
        path: `${sitePathConfig.categoryJobs.path}`,
      },
      {
        name: this.isEditing
  ...
  onInsertCompleted = (res) => {
    ...
    if (res?.result) {
      this.showSuccessConfirmModal({
        onContinueEdit: () => {
          history.push(
            sitePathConfig.categoryJobsUpdate.path.replace(":id", res.id)
          );
        },
      });
...
export default connect(
  mapStateToProps,
  mapDispatchToProps
)(withTranslation(["categoryListPage", "listBasePage"])(CategoryJobsUpdate));
