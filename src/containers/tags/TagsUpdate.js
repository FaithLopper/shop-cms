...
import TagsUpdateForm from "../../compoments/tags/TagsUpdateForm";

class TagsUpdate extends SaveBasePage {
  constructor(props) {
    super(props);
    const { t } = this.props;
    this.objectName = t("objectName");
    this.getListUrl = sitePathConfig.tags.path;
    this.actionFooter = false;
    this.breadcrumbs = [
      {
        name: t("breadcrumbs.parentPage"),
        path: sitePathConfig.tags.path,
      },
      {
        name: this.isEditing
  ...
  onInsertCompleted = (res) => {
    ...
    if (res?.result) {
      this.showSuccessConfirmModal({
        onContinueEdit: () => {
          history.push(
            sitePathConfig.tags.childrenKeys[1].replace(":id", res.id)
          );
        },
      });
...
const mapDispatchToProps = (dispatch) => ({
  ...
  uploadFile: (payload) => dispatch(actions.uploadFile(payload)), //re check
});
...
export default connect(
  mapStateToProps,
  mapDispatchToProps
)(withTranslation(["tagsListPage", "listBasePage"])(TagsUpdate));
