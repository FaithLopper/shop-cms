...
class DropdownFieldWithSearch extends BaseField {
  constructor(props) {
    super(props);
        ...
        >
          {options
            ? options.map((item) => (
                ...
                >
                  {noTranslate ? item[optionLabelKey] : t(item[optionLabelKey])}
                  {labelInValue ? t(item[optionLabelKey]) : null}
                </Select.Option>
              ))
            : null}
        </Select>
...
export default withTranslation(["constants", "baseField"])(
